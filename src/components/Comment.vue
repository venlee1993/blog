            ...
            <div class="d-flex">
              <div class="avator">
                <b-img
                  ...
                  width="32"
                  height="32"
                  :src="`http://blog.com/${comment.user.avatar}`"
                  alt="img"
                ></b-img>
              </div>
                ...
                <div class="comment-temp">
                  ...
                  <div class="reply-stat d-flex">
                    ...
                    <div class="action-box">
                      <span class="like" @click="likeRequest(comment.id)">
                        <i class="fas fa-thumbs-up"></i>0
                      </span>
                      <span class="reply" @click="slideInput(comment.id,0)">
                        <i class="fas fa-comment"></i>
                        {{compare(comment.id,0)?'收起回复':'回复'}}
                      </span>
                    </div>
                  </div>
                  <v-msg
                    v-if="compare(comment.id,0)"
                    :params="{post_id:postId,comment_id:comment.id,reply_user_id:comment.user.id}"
                    @publish="replyComment"
                  ></v-msg>
                </div>
                      ...
                      <div class="d-flex reply-temp">
                        <div class="avator">
                          <b-img
                            ...
                            width="32"
                            height="32"
                            :src="`http://blog.com/${reply.user.avatar}`"
                            alt="img"
                          ></b-img>
                        </div>
                        <div class="comment-info">
                          ...
                          <div class="reply-stat d-flex">
                            ...
                            <div class="action-box">
                              <span class="like" @click="likeRequest(reply.id)">
                                <i class="fas fa-thumbs-up"></i>0
                              </span>
                              <span class="reply" @click="slideInput(comment.id,reply.id)">
                                <i class="fas fa-comment"></i>
                                {{compare(comment.id,reply.id)?'收起回复':'回复'}}
                              </span>
                            </div>
                          </div>
                          <v-msg
                            v-if="compare(comment.id,reply.id)"
                            :params="{post_id:postId,comment_id:comment.id,reply_user_id:reply.user.id}"
                            @publish="replyComment"
                          ></v-msg>
                        </div>
                      </div>
...
export default {
  ...
  methods: {
    slideInput(index, idx) {
      this.slideDownId = this.compare(index, idx)
        ? "0-0"
    ...
    addComment(parmas) {
      this.$http.post("/apis/comment/store", parmas).then(res => {
        if (res.status == 200) {
    ...
    replyComment(params) {
      this.$http.post("/apis/reply/store", params).then(res => {
        console.log(res);
      });
    },
    likeRequest(id) {
      this.$http.post("/apis/like/action", { cid: id }).then(res => {});
    }
  }
};
</script>
